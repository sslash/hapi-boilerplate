...
class CircuitMapper {
    ...
    static multiMapper (rows) {
        ...
        const circuits = _.reduce(rows, (result, current) => {
            ...
            const move = new Move(current.m_title, current.m_description);
            const circuitMove = new CircuitMove(current.c_id, move, current.cm_description);
            const circuit = result[current.c_id] || new Circuit(current.c_title, current.c_description, current.c_tags);
